...
import { createNccAttack, createNccDefense, createNccReveal } from '../packages/sdk/src/ncc-helper.ts';
import { scanForBip39Words } from '../packages/sdk/src/bip39-scanner.ts';
...
async function main() {
  ...
  // Write a narrative with the target word + known BIP39 words
  // (Skip loading all 2048 words — use known BIP39 words directly)
  const knownBip39 = [
    'abandon', 'ability', 'able', 'about', 'above', 'absent', 'absorb', 'abstract',
    'absurd', 'abuse', 'access', 'accident', 'account', 'accuse', 'achieve', 'acid',
    'acoustic', 'acquire', 'across', 'act', 'action', 'actor', 'actual', 'adapt',
    'add', 'addict', 'address', 'adjust', 'admit', 'adult', 'advance', 'advice',
    'afraid', 'again', 'agree', 'ahead', 'aim', 'air', 'airport', 'aisle',
  ...
  const narrative = `The ancient warrior decided to ${targetWord} every obstacle in the path of abandon. With unwavering ability, they would absorb the abstract knowledge hidden across the realm and achieve acoustic harmony above all things.`;
