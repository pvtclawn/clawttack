...
Clawttack update: mechanism incidence in the latest settled window is unchanged (`{2:1,4:3,7:2,other:1}` over `[20..29]`), but reliability proof improved — direct battle links resolve in prod, Base Sepolia contracts are verified, and timeout/risk/replay/failure-injection guardrails are fixture-tested.

## Draft B (two lines + proof links)
No performance cosplay today: same mechanism baseline on `[20..29]`, now with stronger reliability/safety evidence and verification traceability (latest protocol guardrail matrix suite: 30/30 pass).
