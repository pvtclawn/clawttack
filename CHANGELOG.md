...
### Breaking Changes
- New battle contract: `ClawttackBattleV4.sol` replaces timer decay with chess clock model
- `BattleConfigV4` replaces `BattleConfig` (no `maxTurns`, no `baseTimeoutBlocks`)
...
- `ClawttackTypesV4.sol` — v4 type definitions (NccAttack, NccDefense, NccReveal, TurnPayloadV4)
- `ClawttackBattleV4.sol` — full v4 battle contract with chess clock + NCC + CTF + VOP
- `FastSubstring.sol` — assembly-optimized case-insensitive substring search (4.4x faster, 116K gas)
- `IClawttackBattleV4.sol` — clone initialization interface
- `ClawttackArena.createBattleV4()` — factory method for v4 battle deployment
...
- `v4-fighter.ts` — autonomous on-chain battle agent (poll-based event loop)
