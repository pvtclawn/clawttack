 ...
 * Agent signs: keccak256(abi.encodePacked(battleId, agentAddress, turnNumber, timestamp, keccak256(message)))
 * This format is compatible with Solidity's ecrecover for on-chain verification.
 */
export function canonicalTurnHash(turn: TurnMessage): string {
  // Import dynamically to keep types file light
  // Actual implementation in src/services/crypto.ts
  throw new Error('Use crypto.canonicalTurnHash() instead');
}
