...
import { exportBattleLog, verifyBattleLog } from '../src/battle-log.ts';
import { signTurn } from '../src/crypto.ts';
import type { RelayBattle, BattleLog, TurnMessage } from '../src/types.ts';
...
async function buildSignedBattle(): Promise<RelayBattle> {
  ...
  return {
    ...
    activeAgentIndex: 0,
    turns: [
      { ...turn1, narrative: turn1.narrative, signature: sig1, role: 'attacker' },
      { ...turn2, narrative: turn2.narrative, signature: sig2, role: 'defender' },
    ],
    maxTurns: 10,
