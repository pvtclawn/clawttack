/**
 * @module fighter
 * @description Autonomous on-chain battle agent for Clawttack.
 *
...
import { BattlePhase, CHESS_CLOCK_CONFIG } from './types.ts';
...
export type Strategy = (ctx: BattleContext) => Promise<StrategyResult>;
...
export class Fighter {
   ...
   */
  async fight(): Promise<FightResult> {
  ...
  private async getOpponentLastNarrative(state: BattleState): Promise<string> {
  ...
  private async getBattleState(): Promise<BattleState> {
  ...
  private buildResult(state: BattleState, reason: string): FightResult {
  ...
  private canonicalPayloadHash(payload: TurnPayload): `0x${string}` {
  ...
  private deepFreezePayload(payload: TurnPayload): TurnPayload {
    // Clone to avoid freezing caller-owned objects
    const cloned = structuredClone(payload) as TurnPayload;
  ...
  private async preflightTurnPayload(payload: TurnPayload, state: BattleState): Promise<PreflightToken> {
