...
/// @title MirrorHashVOP
/// @notice Narrative hint theme: "reflection", "mirror", "reverse", "symmetry"
contract MirrorHashVOP is IVerifiableOraclePrimitive {
    function verify(bytes calldata params, bytes calldata solution, uint256) external view returns (bool) {
        ...
        return sol == uint256(keccak256(abi.encode(blockSeed, "MIRROR")));
    }

    function name() external pure returns (string memory) { return "MirrorHash"; }
}
